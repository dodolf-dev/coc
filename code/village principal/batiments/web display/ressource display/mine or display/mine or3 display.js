...
function updatemine_orOptions() {
    ...
    mine_orLevels.forEach(([key, data]) => {
        ...
        option.value = level;
        option.textContent = `Mine d'or 3 Niveau ${level}`;
...
function updatemine_or3Info() {
    ...
    document.getElementById("mine_or3_prix_niveau").innerHTML = `Prix restant : ${formatPrix(prixrestant)} <img src="/coc/image/village principal/ressource/elixir village-p.jpg" alt="or" class="icone-ressource">`;
