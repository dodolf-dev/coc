...
function updatebatisseurOptions() {
    ...
    batisseurLevels.forEach(([key, data]) => {
        ...
        option.value = level;
        option.textContent = `Bâtisseur 5 Niveau ${level}`;
...
function updatebatisseur5Info() {
    ...
    if (prixrestant === 0) {
        document.getElementById("batisseur5_prix_niveau").innerHTML = `Prix restant : max <img src="/coc/image/village principal/ressource/or village-p.jpg" alt="or" class="icone-ressource">`;    
    }
    else {
    ...
    if (tempsRestant === 0) {
        document.getElementById("batisseur5_temps_niveau").innerHTML = `Temps restant: max <img src="/coc/image/général/ressource/temps icone.png" alt="temps" class="icone-ressource">`;
    }
    else {
