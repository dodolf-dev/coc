...
const def_anti_air4_box = document.getElementById("def_anti_air4_box");
const selectdef_anti_air4 = document.getElementById("def_anti_air4");
const imagedef_anti_air4 = document.getElementById("image-def_anti_air4");
...
function updatedef_anti_airOptions() {
    ...
    const currentdef_anti_airLevel = parseInt(selectdef_anti_air4.value) || 0; // Récupérer l'ancien niveau

    // Filtrer les niveaux de def_anti_air disponibles en fonction de l'HDV
    const def_anti_airLevels = Object.entries(def_anti_air4)
        .filter(([key, data]) => data.hdvrequis <= hdvLevel)
        .sort((a, b) => a[1].hdvrequis - b[1].hdvrequis);

    // Réinitialiser les options du select
    selectdef_anti_air4.innerHTML = "";
    ...
    def_anti_airLevels.forEach(([key, data]) => {
        ...
        option.value = level;
        option.textContent = `défense anti aérienne 4 Niveau ${level}`;
        selectdef_anti_air4.appendChild(option);
    ...
    if (def_anti_airLevels.length === 0) {
        def_anti_air4_box.style.display = "none";
        selectdef_anti_air4.style.display = "none";
        imagedef_anti_air4.style.display = "none";
        infoContainer.style.display = "none";
    } else {
        def_anti_air4_box.style.display = "block";
        selectdef_anti_air4.style.display = "block";
        imagedef_anti_air4.style.display = "block";
        infoContainer.style.display = "block";
        selectdef_anti_air4.value = selectedLevel;
        updatedef_anti_air4Info();
    }
}

function updatedef_anti_air4Info() {
    const niveau = `def_anti_air4_nv_${selectdef_anti_air4.value}`;
    const data = def_anti_air4[niveau];
    const hdvNiveau = parseInt(document.getElementById("hdv").value, 10);
    const prixrestant = calculerPrixRestantdef_anti_air4(parseInt(selectdef_anti_air4.value, 10),def_anti_air4_nv_max_hdv(hdvNiveau));
    const tempsRestant = calculerTempsRestantdef_anti_air4(parseInt(selectdef_anti_air4.value, 10), def_anti_air4_nv_max_hdv(hdvNiveau));
