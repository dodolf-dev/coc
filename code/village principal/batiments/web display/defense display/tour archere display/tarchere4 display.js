...
function updateCanonOptions() {
    ...
    canonLevels.forEach(([key, data]) => {
        ...
        option.value = level;
        option.textContent = `Tour d'archère 4 Niveau ${level}`;
...
function updatetour_archere4Info() {
    ...
    if (data) {
        ...
        document.getElementById("tour_archere4_prix_niveau").innerHTML = `Prix restant : max <img src="/coc/image/village principal/ressource/or village-p.jpg" alt="or" class="icone-ressource">`;    
    }
    else {
    ...
    if (tempsRestant === 0) {
        document.getElementById("tour_archere4_temps_niveau").innerHTML = `Temps restant: max <img src="/coc/image/général/ressource/temps icone.png" alt="temps" class="icone-ressource">`;
    }
    else {
