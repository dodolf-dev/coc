...
function updateCanonOptions() {
    ...
    canonLevels.forEach(([key, data]) => {
        ...
        option.value = level;
        option.textContent = `Tour de l'enfer 1 Niveau ${level}`;
...
function updatetour_enfer1Info() {
    ...
    if (data) {
        ...
        document.getElementById("tour_enfer1_prix_niveau").innerHTML = `Prix restant : max <img src="/coc/image/village principal/ressource/or village-p.jpg" alt="or" class="icone-ressource">`;    
    }
    else {
    ...
    if (tempsRestant === 0) {
        document.getElementById("tour_enfer1_temps_niveau").innerHTML = `Temps restant: max <img src="/coc/image/général/ressource/temps icone.png" alt="temps" class="icone-ressource">`;
    }
    else {
