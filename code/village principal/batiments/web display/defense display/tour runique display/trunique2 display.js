...
function updateCanonOptions() {
    ...
    canonLevels.forEach(([key, data]) => {
        ...
        option.value = level;
        option.textContent = `Tour runique 2 Niveau ${level}`;
...
function updatetour_runique2Info() {
    ...
    if (data) {
        ...
        document.getElementById("tour_runique2_prix_niveau").innerHTML = `Prix restant : max <img src="/coc/image/village principal/ressource/or village-p.jpg" alt="or" class="icone-ressource">`;    
    }
    else {
    ...
    if (tempsRestant === 0) {
        document.getElementById("tour_runique2_temps_niveau").innerHTML = `Temps restant: max <img src="/coc/image/général/ressource/temps icone.png" alt="temps" class="icone-ressource">`;
    }
    else {
